...
/**
 * space/store.ts（空间端状态中心）
 * - 维护床、C 臂、场景工具等全局状态
 * - 管理后端轮询、熔断与同步策略
 * - 承接 UI 操作与后端数据回读，统一做限位与单位处理
 */
...
import type {
  ...
  Annotation, LogEntry, ToastEntry, XYZ, SpaceDeviceInfo,
...
import { applyThresholdFilter } from './utils/thresholdFilter';
...
const ERROR_RECOVERY_TIME = 5000; // 5秒后重试
    ...
    void loadFromBackend()
      ...
      .catch((err) => {
        ...
        if (_consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
          isCoolingDown = true;
          const breakerMsg = `[Circuit Breaker] Too many consecutive errors (${_consecutiveErrors}), cooling down for ${ERROR_RECOVERY_TIME}ms. Last error: ${errMsg}`;
          console.warn(breakerMsg);
          if (addToast) {
 ...
 */
const toBackendBedHeightStroke = (heightAbsMm: number) => clampHeight(heightAbsMm) - BED_HEIGHT_MIN;
...
const pickCArmFromSpace = (
  space: Record<string, SpaceDevice>,
  selectedCArmDeviceId: string | null
): SpaceDevice | null => {
  const resolved = resolveCArmBinding(Object.values(space) as SpaceDeviceInfo[], selectedCArmDeviceId);
  return resolved ? space[resolved.id] ?? null : null;
};
...
let _awaitingBackendResetTimer: ReturnType<typeof setTimeout> | null = null;
let _lastBackendLoadLogAt = 0;

let _lastUserCommandAt = 0;
const IGNORE_BACKEND_AFTER_COMMAND_MS = 800;

const scheduleSyncToBackend = (sync: () => Promise<void>, delay = 120) => {
  if (_syncDebounceHandle !== null) {
    clearTimeout(_syncDebounceHandle);
  }

  _syncDebounceHandle = setTimeout(() => {
    _syncDebounceHandle = null;
    _lastUserCommandAt = Date.now();
    void sync();
  }, delay);
};
  ...
  persist(
    (set, get) => {
        ...
        if (isLikelyBedDevice(device)) {
          ...
          set(state => ({
            bed: {
              ...
              rotY,
              tilt,
              // 应用阈值过滤，避免小数点后两位的微小变化导致模型闪烁
              height: applyThresholdFilter(
                fromBackendBedHeight(pickBedHeightJointValue(joints, state.bed.height)),
                state.bed.height
              ) ?? state.bed.height,
              trendelenburg: applyThresholdFilter(
                clampBedTrendelenburg(
                  joints.tilt ?? 
              ...
              backrestAngle: applyThresholdFilter(
                clampBedBackrestAngle(
              ...
              leftLegAngle: applyThresholdFilter(
                clampBedLegAngle(
                  joints.leftLegAngle ?? 
              ...
              rightLegAngle: applyThresholdFilter(
                clampBedLegAngle(
          ...
          set(state => ({
            cArm: {
              ...
              rotZ,
              // 应用阈值过滤，避免小数点后两位的微小变化导致模型闪烁
              cArmHeightJoint: applyThresholdFilter(
              ...
              cArmRotation: applyThresholdFilter(
                cj.arm_tilt_joint ?? cj.arm_rotation ?? cj.tilt_joint ?? state.cArm.cArmRotation,
                state.cArm.cArmRotation
              ) ?? state.cArm.cArmRotation,
              cArmFrontBackRotation: applyThresholdFilter(
                cj.c_ring_rotation_joint ?? cj.ring_rotation ?? cj.rotation_joint ?? state.cArm.cArmFrontBackRotation,
                state.cArm.cArmFrontBackRotation
              ) ?? state.cArm.cArmFrontBackRotation,
              frontBackTranslation: applyThresholdFilter(
                clampCArmFrontBackTranslation(
                  normalizeBackendLinear(
      ...
      setInteractionState: (state) => {
        if (_awaitingBackendResetTimer) {
          clearTimeout(_awaitingBackendResetTimer);
          _awaitingBackendResetTimer = null;
        }
        ...
        if (state === 'AWAITING_BACKEND_UPDATE') {
          _awaitingBackendResetTimer = setTimeout(() => {
            const current = get().interactionState;
            if (current === 'AWAITING_BACKEND_UPDATE') {
              set({ interactionState: 'IDLE', interactingPart: null });
            }
            _awaitingBackendResetTimer = null;
          }, 1500);
      ...
      syncToBackend: async () => {
        try {
          const { bed, cArm, lastUpdateSource } = get();
          ...
          const bedPayload: DeviceState = {
            device: 'surgical_bed',
            timestamp: Date.now(),
            state: {
              bed: {
                position: {
                  x: denormalizeLinearForBackend(bed.x),
                  y: denormalizeLinearForBackend(bed.y),
                  z: denormalizeLinearForBackend(bed.z),
                },
                rotation: { x: bed.rotX, y: bed.rotY, z: bed.tilt },
                joints: {
                  bed_height_joint: toBackendBedHeightStroke(bed.height),
                  bed_tilt_joint: bed.trendelenburg,
                  bed_lateral_joint: bed.lateral,
                  bed_panel_back_joint: bed.backrestAngle,
                  bed_panel_right_leg_joint: bed.rightLegAngle,
                  bed_panel_left_leg_joint: bed.leftLegAngle,
                  bed_front_back_joint: denormalizeLinearForBackend(bed.frontBackPosition),
                },
              },
            },
          };

          const cArmPayload: DeviceState = {
            device: 'c_arm',
            timestamp: Date.now(),
            state: {
              cArm: {
                position: {
                  x: denormalizeLinearForBackend(cArm.x),
                  y: denormalizeLinearForBackend(cArm.y),
                  z: denormalizeLinearForBackend(cArm.z),
                },
                rotation: { x: cArm.caud, y: cArm.rao, z: cArm.rotZ },
                joints: {
                  arm_height_joint: denormalizeLinearForBackend(cArm.cArmHeightJoint),
                  arm_tilt_joint: cArm.cArmRotation,
                  c_ring_rotation_joint: cArm.cArmFrontBackRotation,
                  arm_front_back_joint: denormalizeLinearForBackend(cArm.frontBackTranslation),
                },
              },
            },
          };
          ...
          // 直接恢复到 IDLE 状态，控制逻辑应走 moveJoint 或 applyPreset
...
useStore.subscribe((state, prevState) => {
  if (state.lastUpdateSource !== 'ui') return;

  const bedChanged = JSON.stringify(state.bed) !== JSON.stringify(prevState.bed);
  const cArmChanged = JSON.stringify(state.cArm) !== JSON.stringify(prevState.cArm);

  if (!bedChanged && !cArmChanged) return;

  scheduleSyncToBackend(() => useStore.getState().syncToBackend());
});
