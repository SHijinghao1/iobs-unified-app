...
const mapHeightJointToJ1 = (height: number) => {
  // 用户需求：UI 显示 300-400mm，但实际物理行程只需 300-330mm
...
const useHighlightFromInteraction = (interactingPart: string | null): string[] => {
...
const URDF_BED_URL = `${MODEL_BASE_URL}urdf/surgical_bed_v2_optimized.urdf`;
const URDF_C_ARM_URL = `${MODEL_BASE_URL}urdf/c_arm_optimized.urdf`;
...
export const URDFBed: React.FC = () => {
  ...
  const interactionHighlight = useHighlightFromInteraction(interactingPart);
  ...
  return (
    <URDFModel
      ...
      lerpFactor={0.35}
      onLoad={() => setModelLoadProgress('bed', 100)}
    />
  );
};
...
export const URDFCArm: React.FC = () => {
  ...
  const linkOffsets = useMemo((): LinkOffsets => ({
    ...
    // c_arm_ring_arm（斜臂）: 已是 c_arm_head 的子节点，无需重复叠加 frontBackWorld
    c_arm_ring_arm: {
      pos: {
    ...
    // c_arm_ring_no_arm（C环）: 已是 ring_arm 的子节点，无需重复叠加 frontBackWorld
    c_arm_ring_no_arm: {
      pos: {
  ...
  return (
    <>
      ...
      highlightedLinks={highlightedLinks}
      lerpFactor={0.5}
      onLoad={() => setModelLoadProgress('cArm', 100)}
    />
    </>
  );
};
...
export const OperatingRoomScene: React.FC = () => {
  ...
  return (
    <>
      {/* Lighting - optimised: minimal lights for performance */}
      <ambientLight intensity={2.0} color="#ffffff" />
      <hemisphereLight args={[0xffffff, 0x889aaa, 0.8]} />
      ...
      <OrbitControls
        ref={orbitRef} makeDefault enableDamping dampingFactor={0.15}
