...
/**
 * BedTabPanel（空间端）
 * - 负责手术床手动控制、预设姿态与 Demo 演示入口
 * - 负责把 UI 操作转成关节动作请求，并显示同步状态
 * - 与 store 中的交互状态机配合，避免“操作中回读覆盖”
 */
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {typeof speed === 'number' && onSpeedChange && (
            <label className="flex items-center gap-1.5 min-w-0">
              <span className="text-[10px] text-gray-500 shrink-0">速度：</span>
              <input
                type="number"
                min={0}
                step={1}
                ...
                className="w-16 h-6 px-2 rounded border border-gray-700 bg-gray-950 text-[11px] font-mono text-gray-200 outline-none focus:border-neon-cyan"
              />
              {speedUnit && <span className="text-[10px] text-gray-500 shrink-0">{speedUnit}</span>}
            </label>
          )}
          {typeof coordinated === 'boolean' && onCoordinatedChange && (
            <label className="flex items-center gap-1.5 shrink-0">
              <span className="text-[10px] text-gray-500">协动：</span>
              <input
                type="checkbox"
                checked={coordinated}
                onChange={(e) => onCoordinatedChange(e.target.checked)}
                className="h-3.5 w-3.5 rounded border border-gray-600 bg-gray-950 text-neon-cyan focus:ring-0"
              />
            </label>
          )}
        </div>
        <span className="text-[11px] font-mono font-bold text-neon-cyan bg-neon-cyan/5 px-2 py-0.5 rounded border border-neon-cyan/20 shrink-0">
          {shownDecimals > 0 ? shownValue.toFixed(shownDecimals) : shownValue}{shownUnit}
        </span>
      </div>
      <div className="h-8 bg-gray-950 rounded-lg border border-gray-800 group-hover:border-gray-700 flex items-center px-3 relative transition-colors">
...
export const BedTabPanel: React.FC = () => {
  ...
  }, [showManualWindow, loadSpaceDevices, loadFromBackend]);

  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="p-4 border-b border-gray-800 shrink-0 grid grid-cols-4 gap-2">
        {([
